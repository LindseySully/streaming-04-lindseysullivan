...
OUTPUT_CSV_FILE2 = os.path.join(csv_files_directory,"song_message.csv")
...
def callback(ch, method, properties, body):
    ...
    # transform message to split the release date and change from yyyy-mm-dd to dd-mm-yyyy format
    try:

        # split message by commas to extract the release date
        parts = message.split(",")
        release_date = parts[0]

        # Parse the release date in the original format
        original_date = datetime.datetime.strptime(release_date, "%Y-%m-%d")

        # Convert the date to the desired format (dd-mm-yyyy)
        transformed_date = original_date.strftime("%d-%m-%Y")

        # Replace the original release date in the message
        parts[0] = transformed_date

        # Join the parts back together to create the transformed message
        transformed_message = ",".join(parts)
        ...
        print(f" [x] Error transforming message {e}")
